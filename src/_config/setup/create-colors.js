...
const colorsBase = JSON.parse(
	fs.readFileSync("./src/_data/designTokens/colorsBase.json", "utf-8"),
);
...
const generatePalette = (baseColorHex, steps) => {
	...
	return steps.map((step) => {
		const color = new Color("oklch", [
			step.lightness,
			baseColor.c * step.chromaFactor,
			baseColor.h,
		]).to("srgb");

		const [r, g, b] = color.coords.map((value) =>
			Math.round(Math.min(Math.max(value * 255, 0), 255)),
		);
...
colorsBase.light_dark.forEach((color) => {
	...
	const [r, g, b] = subduedColor.coords.map((value) =>
		Math.round(Math.min(Math.max(value * 255, 0), 255)),
	);
...
fs.writeFileSync(
	"./src/_data/designTokens/colors.json",
